...
DoubanBoard = {
    ...
    loadSingleBoard: function () {
        ...
        $.each($(".douban-single"), function (i, item) {
            ...
            if ($(item).attr("data-type") == "book") {
                api = window.DoubanSingleBookAPI + "?type=singlebook&id=" + $(item).attr("data-id");
            } else {
                api = window.DoubanSingleMovieAPI + "?type=singlemovie&id=" + $(item).attr("data-id");
            }
            $.getJSON(api, function (result) {
                ...
                // var html = `<div class="douban-single-img" style="background-image:url(` + result['cover'] + `)"></div>
                var html = `<img class="douban-single-img" src="` + result['cover'] + `" onerror="javascript:this.src='/static/images/notfound.jpg';"/>
    ...
    loadBooks: function (status) {
        ...
        var api = window.DoubanBookAPI + "?type=book&from=" + String(curBooks) + "&status=" + status;
        $.getJSON(api, function (result) {
            $(`#loadMoreBooks_` + status).html("加载更多");
            if (result.length < DoubanPageSize) {
                $(`#loadMoreBooks_` + status).html("没有啦");
            }
            /**<div  class="doubanboard-thumb" style="background-image:url(`+ item[type[index]] + `)"></div>**/
            $.each(result, function (i, item) {
                var html = `<div id="doubanboard-book-item-` + String(curBooks) + `" class="doubanboard-item">
                            <img  class="doubanboard-thumb" src="`+ item['cover'] +`" onerror="javascript:this.src='/static/images/notfound.jpg';"/>
                            <div title="点击显示详情" class="doubanboard-title">`+ item.name + `</div>
    ...
    loadMovies: function (status) {
        ...
        $.getJSON(window.DoubanMovieAPI + "?from=" + String(curMovies) + "&status=" + status, function (result) {
            ...
            /**<div class="doubanboard-thumb" style="background-image:url(`+ item[type[index]] + `)"></div><**/
            $.each(result, function (i, item) {
                var html = `<a href="` + item.link + `" target="_blank" id="doubanboard-movie-item-` + String(curMovies) + `" class="doubanboard-item">
